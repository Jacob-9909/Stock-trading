...
def llm_thinking(analyzer, strategy_name='sma_crossover', max_rows=60, bt_result=None):
    ...
    *전략 백테스트 성과:
        {result_text}
    ---
    위의 심층 데이터를 기반으로, {analyzer.ticker} 종목에 대한 전문가 시각의 종합 투자 분석 리포트를 한국어로 작성하라. 
    
    **중요: 응답은 반드시 마크다운(Markdown) 형식으로 작성해야 한다.**
    
    리포트는 다음 핵심 내용을 포함해야 한다:
    ...
    **작성 가이드라인:**
    ...
    출력 형식: 응답은 반드시 마크다운 형식으로 작성하라. 제목은 ##, 소제목은 ###을 사용하고, 중요한 내용은 **볼드체**로 강조하라. 리스트는 - 또는 1. 형식을 사용하라.
    """
